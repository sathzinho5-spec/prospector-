...
function bindEvents() {
  ...
  $("btnDisparo").addEventListener("click", function () {
    $("disparoCard").classList.remove("hidden");
    refreshDisparo();
    $("disparoCard").scrollIntoView({ behavior: "smooth", block: "nearest" });
  });
  $("btnCloseDisparo").addEventListener("click", function () { $("disparoCard").classList.add("hidden"); });
...
async function refreshDisparo() {
  try {
    ...
    line.textContent = s.rodando
      ? "RODANDO · " + s.enviados_hoje + " hoje · " + s.pendentes + " na fila · próximo: " + (s.proximo_em || "já")
      : "parado · " + s.pendentes + " na fila · " + s.enviados + " enviados";
    line.className = "tag" + (s.rodando ? " on" : "");
    ...
    const rows = (d2.fila || []).map(function (f) {
      return "<tr><td>" + esc(f.nome) + "</td><td>" + esc(f.telefone) + "</td>" +
        ...
        "<td>" + esc((f.mensagem || "").slice(0, 60)) + "…</td></tr>";
    }).join("");
    $("disparoTable").innerHTML = rows
      ? "<table class='disp-table'><thead><tr><th>Lead</th><th>Telefone</th><th>Status</th><th>Mensagem</th></tr></thead><tbody>" + rows + "</tbody></table>"
      : "<p class='hint'>Fila vazia. Clique em 'Enfileirar resultados'.</p>";
