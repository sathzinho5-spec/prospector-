...
function renderCrmBoard() {
  ...
  board.innerHTML = CRM_STAGES.map(function (pair) {
    ...
    html += "<div class='crm-col-head'>" + label + " <span class='tag'>" + cards.length + "</span></div>";
    html += "<div class='crm-col-body'>";
    html += cards.map(function (l) { return crmCard(l); }).join("") || "<p class='hint'>—</p>";
    html += "</div></div>";
    return html;
  }).join("");
}
...
function crmCard(l) {
  ...
  return (
    ...
    (l.foto ? "<img src='" + esc(l.foto) + "' class='crm-avatar' loading='lazy' alt=''>" : "<div class='crm-avatar'>" + esc(initials(l.nome)) + "</div>") +
    "<div style='min-width:0;'><div class='crm-name'>" + esc(l.nome) + "</div>" +
    "<div class='biz-sub2'>" + esc([l.cidade, l.estado].filter(Boolean).join(" - ")) +
    (l.telefone ? " · " + esc(l.telefone) : "") + "</div></div>" +
    scoreHtml +
    "</div>" +
    "<select class='crm-stage' data-id='" + esc(l.id || "") + "' data-nome='" + esc(l.nome || "") + "'>" + opts + "</select>" +
    (l.observacao ? "<div class='crm-obs'>" + esc(l.observacao) + "</div>" : "") +
    "<button class='btn small' style='margin-top:8px;' onclick='editCrmObs(\"" + String(l.id || "").replace(/"/g, "") + "\")'>Anotação</button>" +
    "</div>"
  );
}
...
function renderCrmKpis(arr) {
  ...
  grid.innerHTML =
    stat("Total de leads", total) +
    stat("Novos", novos) +
    stat("Em andamento", andamento) +
    stat("Fechados", fech, fech > 0 ? "ok" : "") +
    stat("Taxa fechamento", taxa + "%", taxa >= 20 ? "ok" : "") +
    stat("Score médio", scored.length ? media + "%" : "—");
