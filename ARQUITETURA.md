...
Teto por arquivo: 350 linhas. Arquivos: 83.

## raiz do projeto

```
app.py                               301  sobe o FastAPI, a porta de acesso e as rotas de busca e ajuste
config.py                             91  caminhos do projeto e leitura das configuracoes, com volume de dados na VPS
...
scheduler.py                         142  agendador da busca recorrente e a comparacao com a rodada anterior
...
rotas/disparo.py                     309  disparo: fila, envio da mensagem, cadencia, iniciar e pausar o motor
...
rotas/modelos.py                     208  os contratos de entrada da API, num lugar so
...
rotas/whatsapp.py                     80  conversas do WhatsApp: chats, mensagens, responder e achar o lead
```
...
scrapers/disparo_abordagens.py       192  o que saiu por lead e a conversa iniciada, fora do alcance da limpeza
...
scrapers/disparo_db.py               218  o banco do disparo: caminho, esquema das tabelas e migracao defensiva
scrapers/disparo_fila.py             266  a fila do disparo em SQLite, o bloqueio de telefone e o anti-duplicata
scrapers/disparo_providers.py        343  quem sabe enviar: Simulado, Evolution API e Meta Cloud API
...
scrapers/google_maps.py              164  conduz a busca no Google Maps: abre o navegador, percorre os cartoes
...
tests/test_disparo_fuso.py            83  prova que o registro de envio grava a hora local mesmo em banco antigo com padrao em UTC
...
tests/test_google_maps_contrato.py    23  trava contra o bug do split 042e1b5, que perdeu o return e fez
```
...
web/js/ajustes.js                    184  ajustes do painel: ler e salvar as configuracoes
...
web/js/busca.js                      276  busca de negocios, filtro dos resultados e exportacao
web/js/conexao.js                     76  conexao do WhatsApp por QR code das instancias Evolution
web/js/contatos.js                    65  quem ja foi contatado e a fila de contato do painel
web/js/conversas.js                  236  conversas do WhatsApp: listar chats, ler e responder
...
web/js/crm_cartao.js                 285  cartao do lead, acao em massa e KPIs do CRM
web/js/detalhe.js                    157  modal de detalhe do lead, screenshot e extracao de contato
web/js/disparo.js                    277  disparo automatico: fila, migracao, envio e status
web/js/disparo_aba.js                280  aba Disparo: os KPIs, a cadencia mostrada e a lista de leads
...
web/js/tabela.js                     278  tabela de resultados: ordenacao, selecao, paginacao e acoes em lote
